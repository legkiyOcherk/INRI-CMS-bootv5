			...
			var id = 'el-finder-io-'+(new Date().getTime()),
				...
				d.append(f.append(e.hide()).prepend(m).append(b)).dialog({
						...
						buttons     : {
							Cancel : function() { $(this).dialog('close'); },
							Ok     : function() {
// cmlex if (!$(':file[value]', f).length) { return error(self.fm.i18n('Select at least one file to upload')); }
								setTimeout(function() {
									self.fm.lock();
										submit();
								});
	...
	image : new function() {
		...
		this.show = function(ql, f) {
			...
			if (ql.mimes[f.mime] && f.hash == ql._hash) {
				$('<img/>').hide().appendTo(ql.media.show()).attr('src', f.url+('?'+Math.random())).load(function() {
					t = $(this).unbind('load');
					if (f.hash == ql._hash) { 
