...
if (isset($_SESSION["WA_USER"])){
  ...
  move_uploaded_file($_FILES['upload']['tmp_name'], "images_ckeditor/".$name);
  $full_path = 'http://'.$_SERVER['SERVER_NAME'].'/images_ckeditor/'.$name;

  $message = "Файл ".$_FILES['upload']['name']." загружен";
  $size=@getimagesize('images_ckeditor/'.$name);
  if($size[0]<50 OR $size[1]<50){
  unlink('images_ckeditor/'.$name);
